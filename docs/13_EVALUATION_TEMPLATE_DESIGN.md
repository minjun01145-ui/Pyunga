...
평가계 담당자는 같은 Section과 교과 입력 데이터를 유지한 채 `표준형`, `공문형`, `간결형` 중 하나를 학교 기본 출력 양식으로 선택한다. 학교명과 교표·로고도 Template의 제한된 표시 설정으로 저장하며, 교사용 미리보기와 인쇄 출력이 같은 설정을 사용한다.

교표는 PNG, JPEG, WebP만 허용하고 브라우저에서 크기를 줄인 뒤 검증된 data URL로 저장한다. 임의 HTML, SVG 또는 외부 이미지 URL은 저장하거나 실행하지 않는다. 출력 양식을 바꾸어도 교과 입력 Schema와 기존 초안의 연결은 바뀌지 않는다.
