...
교사용 전체 평가계획 입력의 비로그인 데모는 Firestore에 저장하지 않는다. 현재 인증 비활성 모드에서는 모든 요청이 동일한 개발 사용자로 해석되므로 서버 저장을 사용하면 여러 방문자가 같은 초안을 덮어쓸 수 있다. 데모 Form Draft는 브라우저 로컬 저장소에 보관하고, 학교 Template만 서버에서 읽는다. 실제 계정 기능을 활성화할 때 사용자/교과 식별자를 확정한 뒤 Admin SDK 기반 사용자별 저장 경로로 교체한다.
