...
인증 비활성 데모 모드는 `NEXT_PUBLIC_AUTHENTICATION_ENABLED`가 `true`가 아닐 때만 사용한다.
운영 전환 시 이 값을 활성화하면 API는 Firebase ID token과 `users/{uid}` 프로필을 검증한다.
...
평가계 담당자는 같은 학교의 일반 교사 계정만 발급하고 비밀번호를 초기화한다.
