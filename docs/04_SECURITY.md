...
로그인 검증용 정보는 기존 해시를 유지한다. 미변경 임시 비밀번호만 인증된 암호화와 Secret Manager 키로 최대 30일 보관해 평가계 사용자가 재확인할 수 있게 한다. 비밀번호 변경·계정 중지 시 전달용 정보는 제거한다.
...
- `PYUNGA_TEMPORARY_PASSWORD_ENCRYPTION_KEY`는 정확히 32바이트의 난수 키를 Base64로 인코딩해 저장
