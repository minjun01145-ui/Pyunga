...
function buildSystemPrompt(): string {
  return [
    ...
    "Section에 입력 양식이 명확히 보이면 config를 제한된 형식으로 제안하세요. 본문 없이 제목만 실제 문서에 표시되는 항목이면 config={type:title_only}로 두고, 구조만 불명확한 경우에만 config를 null로 두세요.",
    "교수학습-평가 표는 type=teaching_learning_table로 두고 orientation, repeatHeader, detailHeaderLabel, fields를 반환하세요. fields의 각 항목은 id, fieldKey, label, inputKind, source, placement(main/detail), widthWeight, required, systemValue를 사용할 수 있습니다. 학사일정에서 자동 입력되는 칸은 source=system으로 두고 systemValue를 반드시 academic_calendar.period, academic_calendar.month, academic_calendar.week, academic_calendar.date_range, academic_calendar.events 중 원문 의미에 맞게 지정하세요.",
...
function buildUserPrompt(sourceText: string, headingHints: readonly string[]): string {
  return [
    ...
    '{"type":"teaching_learning_table","orientation":"landscape","repeatHeader":true,"detailHeaderLabel":"수업-평가 방법, 수업·평가 연계의 주안점","fields":[{"id":"period","fieldKey":"period","label":"시기","inputKind":"text","source":"system","systemValue":"academic_calendar.period","placement":"main","widthWeight":1},{"id":"teaching-methods","fieldKey":"teachingMethods","label":"수업","inputKind":"multiline","source":"teacher","placement":"detail","widthWeight":4}]}',
